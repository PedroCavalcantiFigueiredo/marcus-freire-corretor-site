...
async function getImoveis(params: {
    termo?: string
    tipo?: string
    quartos?: string
}) {
    ...
    // Se o Supabase não estiver configurado, retorna os dados de exemplo filtrados
    if (process.env.NEXT_PUBLIC_SUPABASE_URL === undefined) {
        console.log("[v0] Usando dados de exemplo - banco de dados não configurado")
        let filteredData = IMOVEIS_EXEMPLO
        const { termo, tipo, quartos } = params;
        if (termo) {
            filteredData = filteredData.filter(imovel =>
                imovel.titulo.toLowerCase().includes(termo.toLowerCase()) ||
                imovel.localizacao.toLowerCase().includes(termo.toLowerCase())
            );
        }
        if (tipo) {
            filteredData = filteredData.filter(imovel => imovel.tipo === tipo);
        }
        if (quartos) {
            filteredData = filteredData.filter(imovel => imovel.quartos >= Number(quartos));
        }
        return { data: filteredData, isExample: true }
    }
    ...
    if (params.termo) {
        query = query.or(`titulo.ilike.%${params.termo}%,localizacao.ilike.%${params.termo}%`)
    }
    if (params.tipo) {
...
export default async function ImoveisPage({
    ...
    const { data: imoveis, isExample } = await getImoveis({
        termo: typeof searchParams.termo === 'string' ? searchParams.termo : undefined,
        tipo: typeof searchParams.tipo === 'string' ? searchParams.tipo : undefined,
        quartos: typeof searchParams.quartos === 'string' ? searchParams.quartos : undefined,
    });
